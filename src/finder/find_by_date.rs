...
use rdkafka::{consumer::{BaseConsumer, Consumer}, error::KafkaError, message::BorrowedMessage, Message, Offset, TopicPartitionList};
...
use crate::{cli_structs::Steps, message::{FBMessage, TraceMessage}, Cache, DigitizerMessage, Topics, UnpackMessage};

type Index = i64;
type Timestamp = DateTime<Utc>;

fn get_timestamp_at_index(consumer: &BaseConsumer, topic: &str, index: Index) -> anyhow::Result<Timestamp> {
...
pub(crate) trait FinderByTimestamp<'a> {
    ...
    fn get_next_message_with_timestamp_equal_and_criteria(&self, key: Self::Key, first_message: &Self::Output, timestamp: &Timestamp) -> Option<Self::Output>;
}

fn push_trace(cache: &mut Cache, channel: Channel, dig_trace_msg: &DigitizerAnalogTraceMessage) -> Option<DigitizerId> {
    info!("Found trace");
    if dig_trace_msg.channels().unwrap().iter().find(|c|c.channel() == channel).is_some() {
        info!("Push trace");
...
impl<'a> FinderByTimestamp<'a> for TraceFinderByKafkaTimestamp<'a> {
    ...
    fn get_next_message_with_timestamp_after(&self, timestamp: &Timestamp) -> Option<Self::Output>
    {
        for msg in self.consumer.iter() {
            if let Some(trace) = msg.ok().and_then(TraceMessage::from_borrowed_message).filter(|trace|trace.timestamp() > *timestamp) {
                return Some(trace)
            }
        }
        None
    ...
    fn get_next_message_with_timestamp_equal_and_criteria(&self, cache: &mut Cache, key: Self::Key, first_message: &Self::Output, timestamp: &Timestamp) -> Option<Self::Output> {
        let mut dig_id = None;
        if let Some(did) = push_trace(cache, key, &first_message.get_unpacked_message().expect("")) {
            dig_id = Some(did);
        } else {
            for msg in self.consumer.iter() {
                if let Some(trace) = msg.ok()
                    .and_then(TraceMessage::from_borrowed_message).filter(|trace|trace.timestamp() == *timestamp)
                    .filter(|trace|
                        trace.get_unpacked_message()
                            .and_then(|trace|trace.channels())
                            .and_then(|channels|channels.iter().find(|channel|channel.channel() == key))
                            .is_some()
                        ) {
...
impl<F: FinderByTimestamp> FindByDate<F> {
    ...
    pub(crate) fn find<R>(&self, topics: &Topics, timestamp: Timestamp, key: F::Key) -> anyhow::Result<(Index,F::Output)> {
        let (idx, tmstp) = self.find_first_index_with_timestamp_before(timestamp)?;
        let first_message = self.finder.get_next_message_with_timestamp_after(&timestamp);
        let first_message = self.finder.get_next_message_with_timestamp_equal_and_criteria(key, &timestamp);
    ...
    pub(crate) fn find_first_index_with_timestamp_before(&self, target_timestamp: Timestamp) -> anyhow::Result<(Index,Timestamp)> {
        ...
        for step in (0..self.step.num_step_passes).rev() {
            ...
            info!("Step: {step_size}");
            (index, earliest_timestamp) = self.find_last_index_with_timestamp_after(index, step_size, target_timestamp)?;
        }
