...
use crate::data::{DigitiserMetadata, EventList, Trace};
...
pub(crate) struct BuildGraph<'b, B>
where
    B: Backend<'b>,
{
    width: usize,
...
impl<'b, B> BuildGraph<'b, B>
where
    B: Backend<'b>,
    <B::Backend as DrawingBackend>::ErrorType: 'static,
{
    ...
    #[instrument(skip_all, level = "debug")]
    pub(crate) fn build_trace_graph<'a>(
        &'a self,
        root: &'a DrawingArea<B::Backend, Shift>,
        trace: &Trace,
        eventlist: Option<&EventList>,
    ) -> Result<(), anyhow::Error> {
    ...
    #[instrument(skip_all, level = "debug")]
    fn draw_eventlist_to_chart<'a>(
        &'a self,
        chart: &'a mut ChartContext<B::Backend, Cartesian2d<RangedCoordf32, RangedCoordf32>>,
        eventlist: &EventList,
    ) -> Result<(), anyhow::Error> {
        let data = eventlist
            .iter()
            .map(|el| (el.time as f32, el.intensity as f32));
    ...
    pub(crate) fn build_event_graph(
        &self,
